# V2.0.0 Fix param ngôn ngữ Trang Đăng ký Đại Biểu

## Thêm param để active ngôn ngữ ví dụ lang=vi hay en

```jsx
// Trước khi thay đổi
// src/views/PublicDelegateRegister.tsx

// Sau khi thay đổi
// src/views/PublicDelegateRegister.tsx
```

### Bước 1. Thêm hàm đọc param (đặt ở cấp module, ngay trên export default function PublicDelegateRegister):

```jsx
// Đọc ?lang= từ URL (hỗ trợ nhúng iframe trong WordPress)
const getInitialLang = (): 'vietname' | 'foreign' => {
  try {
    const lang = (new URLSearchParams(window.location.search).get('lang') || '').toLowerCase();
    if (['en', 'foreign', 'international'].includes(lang)) return 'foreign';
    if (['vi', 'vn', 'vietnam', 'vietname'].includes(lang)) return 'vietname';
  } catch {
    /* ignore */
  }
  return 'vietname'; // mặc định tiếng Việt
};
```

### Bước 2. Đổi dòng khởi tạo state:

```jsx
// cũ:
const [nationality, setNationality] = useState<'vietname' | 'foreign'>('vietname');

// mới:
const [nationality, setNationality] = useState<'vietname' | 'foreign'>(getInitialLang);
```

### Bước 3. Ẩn chọn ngôn ngữ
```jsx
// Xóa hoặc comment dòng chọn ngôn ngữ trong file src/views/PublicDelegateRegister.tsx
<div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm">
  <label className="block text-xs font-extrabold text-slate-800 mb-2 uppercase">
    {L.f('nationality', 'Chọn ngôn ngữ *', 'Select Language *')}
  </label>
  <div className="flex bg-slate-200/50 rounded-lg p-1 gap-2 max-w-sm">
    <button
      type="button"
      onClick={() => setNationality('vietname')}
      className={`flex-1 py-2 text-xs font-bold rounded-md transition-all cursor-pointer ${nationality === 'vietname' ? 'bg-teal-900 text-amber-400 shadow-md' : 'text-slate-600 hover:text-slate-900'
        }`}
    >
      {L.t('Việt Nam', 'Vietnamese')}
    </button>
    <button
      type="button"
      onClick={() => setNationality('foreign')}
      className={`flex-1 py-2 text-xs font-bold rounded-md transition-all cursor-pointer ${nationality === 'foreign' ? 'bg-teal-900 text-amber-400 shadow-md' : 'text-slate-600 hover:text-slate-900'
        }`}
    >
      International
    </button>
  </div>
</div>

```

# V2.0.1 Fix param ngôn ngữ Trang Đăng ký Tài trợ

## Thêm param để active ngôn ngữ ví dụ lang=vi hay en

```jsx
// Trước khi thay đổi
// src/views/PublicSponsorRegister.tsx

// Sau khi thay đổi
// src/views/PublicSponsorRegister.tsx
```

### Bước 1. Thêm hàm đọc param (đặt ở cấp module, ngay trên export default function PublicSponsorRegister):

```jsx
// Đọc ?lang= từ URL (hỗ trợ nhúng iframe trong WordPress)
const getInitialLang = (): 'vietname' | 'foreign' => {
  try {
    const lang = (new URLSearchParams(window.location.search).get('lang') || '').toLowerCase();
    if (['en', 'foreign', 'international'].includes(lang)) return 'foreign';
    if (['vi', 'vn', 'vietnam', 'vietname'].includes(lang)) return 'vietname';
  } catch {
    /* ignore */
  }
  return 'vietname'; // mặc định tiếng Việt
};
```

### Bước 2. Đổi dòng khởi tạo state:

```jsx
// cũ:
const [nationality, setNationality] = useState<'vietname' | 'foreign'>('vietname');

// mới:
const [nationality, setNationality] = useState<'vietname' | 'foreign'>(getInitialLang);
```

### Bước 3. Ẩn chọn ngôn ngữ
```jsx
// Xóa hoặc comment dòng chọn ngôn ngữ trong file src/views/PublicDelegateRegister.tsx
<div className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm">
    <label className="block text-xs font-extrabold text-slate-800 mb-2 uppercase">
      {L.f('nationality', 'Chọn ngôn ngữ *', 'Select Language *')}
    </label>
    <div className="flex bg-slate-200/50 rounded-lg p-1 gap-2 max-w-sm">
      <button
        type="button"
        onClick={() => setNationality('vietname')}
        className={`flex-1 py-2 text-xs font-bold rounded-md transition-all cursor-pointer ${nationality === 'vietname' ? 'bg-teal-900 text-amber-400 shadow-md' : 'text-slate-600 hover:text-slate-900'
          }`}
      >
        {L.t('Việt Nam', 'Vietnamese')}
      </button>
      <button
        type="button"
        onClick={() => setNationality('foreign')}
        className={`flex-1 py-2 text-xs font-bold rounded-md transition-all cursor-pointer ${nationality === 'foreign' ? 'bg-teal-900 text-amber-400 shadow-md' : 'text-slate-600 hover:text-slate-900'
          }`}
      >
        International
      </button>
    </div>
  </div>

```

# V2.0.2 Fix bỏ Header
- Trang Đăng ký Đại Biểu: http://localhost:5173/?view=register-delegate&lang=en
- Xóa header trong file src/views/PublicDelegateRegister.tsx

- Trang Đăng ký Tài trợ: http://localhost:5173/?view=register-sponsor&lang=en
- Xóa header trong file src/views/PublicSponsorRegister.tsx

- Trang Đăng ký Báo Cáo: http://localhost:5173/?view=register-speaker&lang=en
- Xóa header trong file src/views/PublicSpeakerRegister.tsx